...
module game::play {
    ...
    public fun step(game_id: ID,index:u8, house_data: &mut HouseData,ctx:&mut TxContext){
        ...
        let mut digest  = bcs::to_bytes(ctx.digest()) ;
        let i = bcs::to_bytes(&index);
        digest.append( i);
        let hash = blake2b256(&digest);

        let lastDigit = digest[hash.length()-1];

        if(game.diffculty==1){
            win = lastDigit < 12;
        }else if(game.diffculty==2){
            win = lastDigit < 10;
        }else if(game.diffculty==3){
            win = lastDigit < 8;
        ...
        // 游戏结束
        finishGame(game_id, house_data, win, index,ctx);
    }
    fun finishGame(game_id: ID, house_data: &mut HouseData,win:bool,index:u8,ctx:&mut TxContext){
        let Game {
            id,
            total_stake,
            player,
